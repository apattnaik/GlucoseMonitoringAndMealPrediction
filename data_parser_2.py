...
def get_data_mapped_by_time(cgm_data, insulin_data):
    ...
    for val1, val1_time in list1:
        min_time_delta = timedelta(minutes=10).total_seconds()
        mapped_val2 = None
        ...
        mapped_data.append((mapped_val2,val1,val1_time))
    return mapped_data
...
def insulin_to_meal():
    ...
    while i<len(sorted_data):
        ...
        if row[1]>1:
            ...
            mapped_label.append((row[0], row[2], 1, 0))
            for j in range(len(sorted_data)):
                if i!=j:
                    if datetime.strptime(sorted_data[j][2],'%Y-%m-%d %H:%M:%S')<end_time:
                        mapped_label.append((sorted_data[j][0],sorted_data[j][2],0,1))
                    else:
                        break
            i = i + j
        else:
            mapped_label.append((row[0],row[2],0,0))
            i = i+1
